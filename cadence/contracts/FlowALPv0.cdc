...
access(all) contract FlowALPv0 {
    ...
    */
    ...
    access(all) resource InternalPosition {
        ...
        /// Returns the collateral token type for this position, or nil if no collateral exists
        access(EImplementation) view fun getCollateralType(): Type? {
        ...
        /// Returns the debt token type for this position, or nil if no debt exists
        access(EImplementation) view fun getDebtType(): Type? {
            for type in self.balances.keys {
    ...
    access(all) struct TokenState {
        ...
        /// For both regular tokens and MOET: Adds tokens to the shared reserve vault.
        access(EImplementation) fun depositCollateral(
            vault: @{FungibleToken.Vault},
            reserveVault: auth(FungibleToken.Withdraw) &{FungibleToken.Vault}
        ) {
            pre {
                vault.getType() == self.tokenType: "Vault type must match token state type"
                vault.getType() == reserveVault.getType(): "Vault and reserve vault types must match"
            }
            reserveVault.deposit(from: <-vault)
        }

        /// Deposits repayment tokens.
        /// For regular tokens: Adds tokens to the shared reserve vault.
        /// For MOET: Burns the tokens.
        access(EImplementation) fun depositRepayment(
            vault: @{FungibleToken.Vault},
            reserveVault: auth(FungibleToken.Withdraw) &{FungibleToken.Vault},
            minter: &MOET.Minter?
        ) {
            ...
            if self.tokenType == Type<@MOET.Vault>() {
                // For MOET, burn the tokens
                Burner.burn(<-vault)
            } else {
                // For other tokens, add to reserve vault
                reserveVault.deposit(from: <-vault)
            }
        }

        /// Withdraws debt tokens.
        /// For regular tokens: Removes tokens from the shared reserve vault.
        /// For MOET: Mints new tokens.
        access(EImplementation) fun withdrawDebt(
            amount: UFix64,
            reserveVault: auth(FungibleToken.Withdraw) &{FungibleToken.Vault},
            minter: &MOET.Minter?
        ): @{FungibleToken.Vault} {
            pre {
                reserveVault.getType() == self.tokenType: "Reserve vault type must match token state type"
            }

            if self.tokenType == Type<@MOET.Vault>() {
                // For MOET, mint new tokens
                assert(minter != nil, message: "MOET minter is required for MOET debt withdrawal")
                return <- minter!.mintTokens(amount: amount)
            } else {
                // For other tokens, withdraw from reserve vault
                assert(reserveVault.balance >= amount, message: "Insufficient reserves for \(self.tokenType.identifier): need \(amount), have \(reserveVault.balance)")
                return <- reserveVault.withdraw(amount: amount)
            }
        }

        /// Withdraws collateral tokens.
        /// For both regular tokens and MOET: Removes tokens from the shared reserve vault.
        access(EImplementation) fun withdrawCollateral(
            amount: UFix64,
            reserveVault: auth(FungibleToken.Withdraw) &{FungibleToken.Vault}
        ): @{FungibleToken.Vault} {
            pre {
                reserveVault.getType() == self.tokenType: "Reserve vault type must match token state type"
                reserveVault.balance >= amount: "Insufficient reserves for \(self.tokenType.identifier): need \(amount), have \(reserveVault.balance)"
            }
            return <- reserveVault.withdraw(amount: amount)
        }
    ...
    access(all) resource Pool {
        ...
        access(self) fun _depositEffectsOnly(
            ...
            if position.balances[type] == nil {
                // Validate that this deposit doesn't create a second collateral type
        ...
        access(EPosition) fun withdrawAndPull(
            ...
            // If this position doesn't currently have an entry for this token, create one.
            if position.balances[type] == nil {
                // Withdrawing from a position with no existing balance will create debt
                // Validate that this doesn't create a second debt type
            ...
            // Get or create reserve vault for this token type
            if self.reserves[type] == nil {
                self.reserves[type] <-! DeFiActionsUtils.getEmptyVault(type)
            }
            let reserveVault = (&self.reserves[type] as auth(FungibleToken.Withdraw) &{FungibleToken.Vault}?)!

            // Withdraw tokens based on whether this is a debt or collateral withdrawal
            // After recordWithdrawal, the balance direction tells us what type of withdrawal this is
            var withdrawn: @{FungibleToken.Vault}? <- nil
            if position.balances[type]!.direction == BalanceDirection.Debit {
                // This is a debt withdrawal (borrowing)
                withdrawn <-! tokenState.withdrawDebt(amount: amount, reserveVault: reserveVault, minter: FlowALPv0._borrowMOETMinter())
            } else {
                // This is a collateral withdrawal (reducing credit)
                withdrawn <-! tokenState.withdrawCollateral(amount: amount, reserveVault: reserveVault)
            }
