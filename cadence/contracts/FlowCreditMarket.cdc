...
access(all) contract FlowCreditMarket {
    ...
    /// ERebalance
    ///
    /// Entitlement for performing automated and manual rebalancing operations on positions.
    /// This entitlement grants access to functions that adjust a position’s internal state in
    /// response to price movements, collateral changes, or strategy thresholds, including
    /// shifting exposure, realizing gains, or reducing risk.
    ///
    /// Rebalancing operations are expected to be mechanically defined and strategy-driven,
    /// rather than discretionary user actions, and may be invoked by scheduled callbacks
    /// or internal protocol logic.
    ///
    /// This entitlement is intentionally narrower than EPosition and is designed to
    /// separate routine, repeatable rebalancing behavior from broader position management
