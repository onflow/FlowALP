...
fun test_multi_asset_liquidation_collateral_selection() {
    ...
    var pid = (openEvents[openEvents.length - 1] as! FlowALPEvents.Opened).pid
    provideSource(signer: usdfLp, positionId: pid, source: nil)
    ...
    pid = (openEvents[openEvents.length - 1] as! FlowALPEvents.Opened).pid
    provideSource(signer: user, positionId: pid, source: nil)
