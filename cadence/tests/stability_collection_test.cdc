...
fun test_collectStability_zeroDebitBalance_returnsNil() {
    ...
    let initialBalance = getStabilityFundBalance(tokenTypeIdentifier: defaultTokenIdentifier)
...
fun test_collectStability_tinyAmount_roundsToZero_returnsNil() {
    ...
    let lp = Test.createAccount()
    setupMoetVault(lp, beFailed: false)
    mintMoet(signer: protocolAccount, to: lp.address, amount: 100.0, beFailed: false)
    ...
    createWrappedPosition(signer: borrower, amount: 1.0, vaultStoragePath: flowVaultStoragePath, pushToDrawDownSink: true)

    // set a very low stability fee rate
    let rateResult = setStabilityFeeRate(signer: protocolAccount, tokenTypeIdentifier: defaultTokenIdentifier, stabilityFeeRate: 0.0001) // 0.01%
    Test.expect(rateResult, Test.beSucceeded())
...
fun test_collectStability_multipleTokens() {
    ...
    let moetRateResult = setStabilityFeeRate(signer: protocolAccount, tokenTypeIdentifier: defaultTokenIdentifier, stabilityFeeRate: 0.1) // 10%
    Test.expect(moetRateResult, Test.beSucceeded())

    let flowRateResult = setStabilityFeeRate(signer: protocolAccount, tokenTypeIdentifier: flowTokenIdentifier, stabilityFeeRate: 0.05) // 5%
    Test.expect(flowRateResult, Test.beSucceeded())

    // verify initial state
    let initialMoetStabilityBalance = getStabilityFundBalance(tokenTypeIdentifier: defaultTokenIdentifier)
    Test.assertEqual(nil, initialMoetStabilityBalance)
    let initialFlowStabilityBalance = getStabilityFundBalance(tokenTypeIdentifier: flowTokenIdentifier)
    ...
    var res = collectStability(signer: protocolAccount, tokenTypeIdentifier: defaultTokenIdentifier)
    Test.expect(res, Test.beSucceeded())

    let balanceAfterMoetCollection = getStabilityFundBalance(tokenTypeIdentifier: defaultTokenIdentifier)
    ...
    let moetLastCollectionTime = getLastStabilityCollectionTime(tokenTypeIdentifier: defaultTokenIdentifier)
    let flowLastCollectionTimeBeforeFlowCollection = getLastStabilityCollectionTime(tokenTypeIdentifier: flowTokenIdentifier)
    ...
    res = collectStability(signer: protocolAccount, tokenTypeIdentifier: flowTokenIdentifier)
    Test.expect(res, Test.beSucceeded())

    let flowBalanceAfterCollection = getStabilityFundBalance(tokenTypeIdentifier: flowTokenIdentifier)
    Test.assert(flowBalanceAfterCollection! > 0.0, message: "Flow stability fund should have received tokens after Flow collection")

    let flowLastCollectionTimeAfter = getLastStabilityCollectionTime(tokenTypeIdentifier: flowTokenIdentifier)
...
fun test_collectStability_zeroRate_returnsNil() {
    ...
    let rateResult = setStabilityFeeRate(signer: protocolAccount, tokenTypeIdentifier: defaultTokenIdentifier, stabilityFeeRate: 0.0)
    Test.expect(rateResult, Test.beSucceeded())
