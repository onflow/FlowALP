...
//
// The MockFlowALPv1Consumer.PositionWrapper provides the necessary
...
transaction(positionWrapperPath: StoragePath) {
    ...
    let positionRef: auth(FungibleToken.Withdraw) &FlowALPv1.Position
    ...
    prepare(borrower: auth(BorrowValue) &Account) {
        ...
        let wrapperRef = borrower.storage.borrow<&MockFlowALPv1Consumer.PositionWrapper>(
