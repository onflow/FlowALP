...
/// Pool.borrowConfig / setWarmupSec. The Pool is obtained via the public capability
/// published at FlowALPv0.PoolPublicPath — no stored cap is required, so any account
/// can run this transaction. It fails at Cadence check time because borrowConfig
/// requires EGovernance, which an unauthorized &Pool does not carry.
transaction(warmupSec: UInt64, poolAddress: Address) {
