...
fun test_FixedRateInterestCurve_uses_spread_model() {
    ...
    let expectedDebitRate = FlowALPv1.perSecondInterestRate(yearlyRate: debitRate)
    ...
    let expectedCreditRate = FlowALPv1.perSecondInterestRate(yearlyRate: expectedCreditYearly)
...
fun test_KinkCurve_uses_reserve_factor_model() {
    ...
    let expectedCreditRate =  FlowALPv1.perSecondInterestRate(yearlyRate: 0.04745)
    Test.assertEqual(expectedCreditRate, tokenState.currentCreditRate)
}
